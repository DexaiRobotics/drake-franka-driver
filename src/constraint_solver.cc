...
namespace franka_driver {
    ...
    ConstraintSolver::ConstraintSolver( const RobotParameters *params)
                                    : p_(params)
                                    , urdf_path_(params->urdf_filepath)
    {
        log()->debug("CS:ConstraintSolver: ctor: BEGIN");
        if (urdf_path_.empty()) {
            std::string error_msg = "CS::ConstraintSolver constructor: empty parameters->urdf_filepath: "
                                + urdf_path_; // add it anyway
            log()->error(error_msg);
            throw std::runtime_error(error_msg);
        }
        if ( ! utils::file_exists(urdf_path_)) {
            std::string error_msg = "CS::ConstraintSolver: URDF NOT FOUND: " + urdf_path_;
            log()->error(error_msg);
            throw std::runtime_error(error_msg);
        }
        ...
        robot_model_idx_ = parser_->AddModelFromFile(
        ...
        std::vector<BodyIndex> robot_body_indices = robots_plant_->GetBodyIndices(robot_model_idx_);
        ...
        plant_context_ =
        &diagram_->GetMutableSubsystemContext(*robots_plant_, &simulator_->get_mutable_context());
        ...
        try {
            // NOTE: The number of bodies, number of joints/DOF, and so on, are set in RBT here.
            drake::parsers::urdf::AddModelInstanceFromUrdfFile( urdf_path_
    ...
    Eigen::MatrixXd ConstraintSolver::GetJointLimits() const
    {
        ...
        for (size_t j = 0; j < joint_names.size(); j++) {
            joint_limits(j, 0) = GetRigidBodyTreeRef().FindChildBodyOfJoint(joint_names.at(j))->getJoint().getJointLimitMin()[0];
            joint_limits(j, 1) = GetRigidBodyTreeRef().FindChildBodyOfJoint(joint_names.at(j))->getJoint().getJointLimitMax()[0];
        }
        return joint_limits;
    }

    std::pair<Eigen::VectorXd, Eigen::VectorXd> ConstraintSolver::GetJointLimitsVectorXdPair() const
    {
        auto joint_names = GetJointNames();
        Eigen::VectorXd lower_limits(num_actuatable_joints_);
        Eigen::VectorXd upper_limits(num_actuatable_joints_);
        for (size_t j = 0; j < joint_names.size(); j++) {
            double lower_limit = GetRigidBodyTreeRef().FindChildBodyOfJoint(joint_names.at(j))->getJoint().getJointLimitMin()[0];
            lower_limits(j) = (double) round(lower_limit*1e4) / 1.0e4;
            double upper_limit = GetRigidBodyTreeRef().FindChildBodyOfJoint(joint_names.at(j))->getJoint().getJointLimitMax()[0];
