...
bool yes_no_prompt(const std::string& beg_prompt) {
  ...
  // NOTE 2: Beware that command line I/O buffering differs between Darwin and
  // Ubuntu
...
// The epoch_seconds argument is expected to ba a count of seconds since 1970
// (Unix time).
std::string date_time_string(time_t epoch_seconds) {
  time_t time = epoch_seconds ? epoch_seconds : std::time(nullptr);
  std::tm* stm =
      std::localtime(&time);  // TODO #time: standardize on localtime or UTC
...
bool sub_dir_path(std::string& sub_dir_path_out,
  ...
  try {
    ...
    dexai::log()->error("Error in sub_dir_path (sub_dir_path_out={}): {}",
                        sub_dir_path_out, ex.what());
  }
  return false;
}
...
std::string get_cwd() {
  ...
  if (!cwd) {
    ...
    dexai::log()->error("Error in get_cwd from getcwd(): {}", error);
    return "";
  }
...
// error. If path_str identifies an existing directory or file, this function
// does not change it.
int make_dirs(const std::string& path_str) {
  ...
  if (mkdir(_path, S_IRWXU) != 0) {
    if (errno != EEXIST) {
      char* error = strerror(errno);
      dexai::log()->error("Error in make_dirs from mkdir({}): {}", path_str,
                          error);
      return -2;
    }
  }
...
bool kill_process(int pid) {
  if (!process_is_alive(pid)) {
    std::cout << "Process with PID " << pid << " is already terminated"
              << std::endl;
    return true;
  } else {
    ...
    usleep(1e6);
    if (!process_is_alive(pid)) {
      std::cout << "Successfully killed PID " << pid << " with SIGTERM"
                << std::endl;
      return true;
    } else {
      ...
      usleep(1e6);
      if (!process_is_alive(pid)) {
        std::cout << "Successfully killed PID " << pid << " with SIGKILL"
                  << std::endl;
        return true;
      }
    }
  }
  std::cerr << "dru:kill_process: failed to kill PID " << pid << "?"
            << std::endl;
  return false;
}

bool lock_pid_file(const std::string& pid_file_path, bool kill_existing_process,
