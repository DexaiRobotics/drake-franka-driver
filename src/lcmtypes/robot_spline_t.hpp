...
const char* robot_spline_t::getTypeName() { return "robot_spline_t"; }
...
int robot_spline_t::_encodeNoHash(void* buf, int offset, int maxlen) const {
  ...
  tlen =
      this->piecewise_polynomial._encodeNoHash(buf, offset + pos, maxlen - pos);
  if (tlen < 0)
    return tlen;
  else
    pos += tlen;
  ...
  for (int a0 = 0; a0 < this->dof; a0++) {
    tlen =
        this->robot_joints[a0]._encodeNoHash(buf, offset + pos, maxlen - pos);
    if (tlen < 0)
      return tlen;
    else
      pos += tlen;
...
int robot_spline_t::_decodeNoHash(const void* buf, int offset, int maxlen) {
  ...
  tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos,
                                &__robot_name_len__, 1);
  if (tlen < 0)
    return tlen;
  else
    pos += tlen;
  if (__robot_name_len__ > maxlen - pos) return -1;
  this->robot_name.assign(static_cast<const char*>(buf) + offset + pos,
                          __robot_name_len__ - 1);
  ...
  tlen =
      this->piecewise_polynomial._decodeNoHash(buf, offset + pos, maxlen - pos);
  if (tlen < 0)
    return tlen;
  else
    pos += tlen;
  ...
  for (int a0 = 0; a0 < this->dof; a0++) {
    tlen =
        this->robot_joints[a0]._decodeNoHash(buf, offset + pos, maxlen - pos);
    if (tlen < 0)
      return tlen;
    else
      pos += tlen;
...
uint64_t robot_spline_t::_computeHash(const __lcm_hash_ptr* p) {
  ...
  for (fp = p; fp != NULL; fp = fp->parent)
    if (fp->v == robot_spline_t::getHash) return 0;
