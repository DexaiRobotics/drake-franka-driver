...
TEST (LoadParams, load_params)
{
    ...
    // EXPECT_NO_THROW( loaded = util_test::load_params(empty_yaml_path, params, verbose));
    EXPECT_TRUE( loaded );
...
TEST (SaveParams, ParametersLog)
{
    ...
    EXPECT_NO_THROW( loaded = util_test::load_params(empty_yaml_path, params, verbose));
    EXPECT_TRUE( loaded );
    const std::string& yaml_test_file_full_path = util_test::get_test_params_yaml_file_name();
...
TEST (LoadParams, MissingAllParamsException)
{
    ...
    int verbose = 2;
    bool loaded = true;
    EXPECT_ANY_THROW( loaded = util_test::load_params(bogus_path, params, verbose) );
    EXPECT_TRUE( loaded );  // still true because not re-set

    // once again, direct call:
    EXPECT_THROW( params = franka_driver::loadYamlParameters(bogus_path, verbose), std::runtime_error);
...
TEST (LoadParams, MissingFoundConverted)
{
    ...
    EXPECT_ANY_THROW( str_val = config["Thandie_Newton_plays_Maeve"].as<std::string>() );

    // yaml-cpp wrapped in getters
    int verbose = 2;
    bool got_val = franka_driver::get_yaml_val_verbose(params, non_key, dbl_val, verbose);
    EXPECT_FALSE( got_val );
    EXPECT_EQ( dbl_val, std::numeric_limits<double>::max() );

    std::string dbl_key = "yaml_test_dbl";
    got_val = franka_driver::get_yaml_val_verbose(params, dbl_key, dbl_val, verbose);
    EXPECT_TRUE( got_val );
    EXPECT_TRUE( utils::EpsEq(dbl_val, 9.87654321) );

    std::string key = "gravity_vector";
    std::vector<double> vec_val{ M_SQRT2, M_E, M_PI };
    log()->info( "YAML value for {} as std::vector<double>, before get: ({} {} {})"
                      , key, vec_val[0], vec_val[1], vec_val[2]);
    got_val = franka_driver::get_yaml_val(params, "gravity_vector", vec_val);
    log()->info( "YAML value for {} as std::vector<double>,  after get: ({} {} {})"
                      , key, vec_val[0], vec_val[1], vec_val[2]);
    EXPECT_TRUE( got_val );
    EXPECT_EQ( vec_val[0], 0.0 );

    Eigen::Vector3d gravity_vector{ M_PI, M_PI_2, M_PI_4 };
    log()->info( "YAML value for {} as Eigen::Vector3d, before get: ({})"
                      , key, gravity_vector.transpose());
    got_val = franka_driver::get_yaml_val(params, "gravity_vector", gravity_vector);
    log()->info( "YAML value for {} as Eigen::Vector3d,  after get: ({})"
                      , key, gravity_vector.transpose());
    EXPECT_TRUE( got_val );
    EXPECT_EQ( gravity_vector[0], 0.0 );

    log()->error("This is a test of dexai::Logger::log_id_size:\n{:>{}}"
