...
private:
    ...
    IKoptions                               *ik_options_        = nullptr;  // FIXME @sprax: don't use a pointer.
...
public:
    ...
    inline RigidBodyTree<double>  *GetRigidBodyTreePtr() const { return  rigid_body_tree_.get(); }
    inline RigidBodyTree<double>&  GetRigidBodyTreeRef() const { return *rigid_body_tree_.get(); }
    inline unsigned             GetNumActuatableJoints() const { return num_actuatable_joints_; }
    inline const IKoptions                         *GetIkOptions()   { return ik_options_; }
    const std::string& GetUrdfPath() const { return urdf_path_; }
    std::vector<std::string> GetJointNames() const;
    Eigen::MatrixXd GetJointLimits() const;
    inline Eigen::MatrixXd GetJointLimitsMatrixXd() const { return GetJointLimits(); }
    std::pair<Eigen::VectorXd, Eigen::VectorXd> GetJointLimitsVectorXdPair() const;
