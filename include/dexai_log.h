...
public:
    template<class It>
    DexaiLogger(const std::string& name, const std::string& log_dir, const It &begin, const It &end)
    ...
    /// like len("[2019-10-28 12:35:00.981] [dexai] [error]")
...
private:
    ...
    unsigned log_id_error_size_ = 42;   // based on "[2019-10-27 14:14:48.870] [dexai] [error] "
};
...
namespace dexai {
    ...
    /// Convenience global function to get the "current" global dexai logger's log directory.
    /// If the global logger (the one returned by dexai::log()) has not yet been created,
    /// calling this will get it created (with warnings about default settings).
    /// If the current logger is not an instance of DexaiLogger, thie function will return
    ...
    ///     at start and finish of a Parlor + StuffGetter session.
    ///
    bool create_log( const std::string& program = "dexai"
                   , const std::string& base_path = ""    // If this is empty, we use the user's HOME/log_robot directory.
                   , const std::string& prefix = ""  // For use in format: [timestamp] [prefix] [level] message
                   , bool use_stdout = false
                   , bool create_once = true
